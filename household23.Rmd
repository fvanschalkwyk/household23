...
# How do the languages vary per province?

  df %>%
  
    select(prov, Languages) %>%
  
    group_by(prov, Languages) %>%
  ...
  # Plot the distribution of population groups between the provinces
    ...
    ggplot(aes(y = fct_reorder(prov, count, sum), x = count, fill = Languages)) +
  
      theme_minimal() +
  
      labs(
        title = "Distribution of primary language by province",
...
# Age distribution of individuals with matric certificate

df %>%
  
  mutate(education = education %>% fct_relevel(
  c(
    "UNSPECIFIED",
    "DO NOT KNOW",
    "NO SCHOOLING",
...
# How does education level differ by age group?
    
    select(Population, age_group, education) %>%
  
    filter(education %>% str_detect("MATRIC")) %>%
  
    group_by(age_group, Population) %>%
  
    summarise(count = n()) %>%
  
  # Plot these data
  
    ggplot(aes(x = Population, y = count, fill = age_group)) +
      ...
      labs(
        ...
        fill = "Age Group",
        caption = "(General Household Survey, 2023. Statistics South Africa)",
        title = "Age distribution of individuals with matric certificate or equivalent by population group"
        ) +
...
# Age distribution of individuals with matric standardised by population group
...
# How does education level differ by age group?
  ...
  # Filter for only matric
    ...
    ungroup(age_group) %>% group_by(Population) %>%
  
    mutate(percentage = count / sum(count) * 100) %>%
  
  # Plot these data in a heatmap
  
    ggplot(aes(x = Population, y = percentage, fill = age_group)) +
  
      theme_minimal() +
  
      labs(
        x = "Population",
        y = "Proportion of population group (%)",
        fill = "Age Group",
        caption = "(General Household Survey, 2023. Statistics South Africa)",
        title = "Age distribution of individuals with matric certificate or equivalent by population group"
        ) +
...
# Relationship between age and education level for each population group
...
# How does education level differ by age group?
    ...
    select(Population, age_group, education, Sex) %>%
  
    filter(education %>% str_detect("NO SCHOOLING")) %>%
  
    group_by(age_group, Population, Sex) %>%
  ...
  ggplot(aes(x = percentage, y = age_group, fill = Sex)) +
  
    theme_minimal() +
  
    labs(
      x = "Proportion of individuals surveyed",
      y = "Age Group",
      caption = "(General Household Survey, 2023. Statistics South Africa)",
      title = "Age distribution of individuals without formal schooling"
    ) +
...
df %>%
  ...
  # Change the order of the education data
...
# Change the names of the factor levels to make them simpler to read
  ...
  mutate(education = education %>% fct_recode(
    ...
    "Grade R/0" = "GRADE R/0",
    "Grade 1" = "GRADE 1/SUB A/CLASS 1",
    "Grade 2" = "GRADE 2/SUB B/CLASS 2",
    "Grade 3" = "GRADE 3/STANDARD 1/ABET/AET 1",
    "Grade 4" = "GRADE 4/STANDARD 2",
    "Grade 5" = "GRADE 5/STANDARD 3/ABET/AET 2",
    "Grade 6" = "GRADE 6/STANDARD 4",
    "Grade 7" = "GRADE 7/STANDARD 5/ABET/AET 3",
    "Grade 8" = "GRADE 8/STANDARD 6/FORM 1",
    "Grade 9" = "GRADE 9/STANDARD 7/FORM 2/ABET/AET 4/NCV LEVEL 1/OCCUPATIONAL CERTIFICATE - NQF LEVEL 1",
    "Grade 10" = "GRADE 10/STANDARD 8/FORM 3/NCV LEVEL 2/OCCUPATIONAL CERTIFICATE - NQF LEV EL 2",
    "Grade 11" = "GRADE 11/STANDARD 9/FORM 4/NCV LEVEL 3/OCCUPATIONAL CERTIFICATE NQF LEV EL 3",
    "Certificate with less than Grade 12" = "CERTIFICATE WITH LESS THAN GRADE 12/STANDARD 10",
    "Diploma with less than Grade 12" = "DIPLOMA WITH LESS THAN GRADE 12/ STANDARD 10",
    "Grade 12" = "GRADE 12/STANDARD 10/FORM 5/ NATIONAL SENIOR CERTIFICATE/ MATRIC/ NCV LEVEL 4 /OCCUPATIONAL CERTIFICATE – NQF LEVEL 4",
    "Certificate with Grade 12" = "HIGHER/NATIONAL/ADVANCE CERTIFICATE WITH GRADE 12/STD 10/ OCCUPATIONAL CERTIFICATE – NQF LEVEL 5",
    "Diploma with Grade 12" = "DIPLOMA WITH GRADE 12/STANDARD 10/OCCUPATIONAL CERTIFICATE – NQF LEVEL 6",
    "Higher Diploma" = "HIGHER DIPLOMA/OCCUPATIONAL CERTIFICATE (B-TECH DIPLOMA) – NQF LEVEL 7",
    "Bachelor's Degree" = "BACHELOR’S DEGREE/OCCUPATIONAL CERTIFICATE – NQF LEVEL 7",
    "Honours Degree" = "HONOURS DEGREE/POSTGRADUATE DIPLOMA/OCCUPATIONAL CERTIFICATE NQF LEVEL 8",
    "Master's Degree" = "POST HIGHER DIPLOMA (M-TECH AND MASTER'S DEGREE) – NQF LEVEL 9",
    "Doctoral Degree" = "DOCTORAL DEGREES (D-TECH AND PHD)– NQF LEVEL 10",
    "Other" = "OTHER THAN ANY OF THE ABOVE"
...
# How does education level differ by age group?
    ...
    group_by(Population, age_group, education) %>%
  
    summarise(count = n()) %>%
  
    mutate(percentage = count / sum(count) * 100) %>%
  
  # Show only matric results
  
    filter(education == "Grade 12") %>%
  
  # Create a plot that shows how the level of education differs with age group

  ggplot(aes(x = age_group, y = percentage, colour = population)) +
...
# How does medical aid usage compare between age groups and population groups?

# Medical Aid is a Yes/No response - presence/absence
# Age groups are binned into about 11 categories (discrete ordinal)
# Population groups are discrete and unordered
# Frequency is an integer/count

# How many people have medical aid and how many don't in each population group, in each age group, and in both population and age groups?
  ...
  # In each age group, how many people have and don't have medical aid?
  
  select(age_group, hlt_medi, Population) %>%
  
  group_by(age_group, hlt_medi, Population) %>%
  
  summarise(count = n()) %>%
  
  ungroup(hlt_medi) %>%
  
  mutate(percentage = count / sum(count) * 100) -> df1

  df1 %>%
    
    group_by(Population, age_group) %>%
    
    mutate(percentage = count / sum(count) * 100) -> df2
  
# Plot the proportion of medical aid users per age group
  
  ggplot(df1, aes(x = percentage, y = age_group, fill = hlt_medi)) +
  ...
  # Plot this pattern for each population group
  
  ggplot(df2, aes(x = percentage, y = age_group, fill = hlt_medi)) +
    
    theme_minimal() +
    
    labs(
      x = "% of individuals surveyed",
      caption = "(General Household Survey, 2023. Statistics South Africa)",
      fill = "Medical Aid"
    ) +
